# -*- coding:utf-8 -*-
'''
由于上个版本在保存数据的地方异常缓慢，需要改写和优化
将连接数据库的地方，进行优化，多线程和保存数据规模化等
'''
import os
import re
...
if __name__ =="__main__":
    ...
    config_df = pd.read_excel(r"{}\city_config.xls".format(base_dir))
    ...
    for one_row in city_month_df.iterrows():
        ...
        if new_cbind:
            ...
            new_df = pd.DataFrame(one_month_data)
            result_df = new_df.merge(config_df, on=['city'])
            result_df.drop_duplicates(inplace=True)
            df_lst.append(result_df)
            print('{}--{}的数据，已经保存'.format(one_city, year_month))
    result_1000_df = pd.concat(df_lst, sort=False)
    result_1000_df.to_pickle('total_data.pkl')