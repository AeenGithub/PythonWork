    ...
    if slider:
        ...
        try:
            ...
            if error:
                ...
                # print(page.url)
                await get_cookie(page)
    await taobao_home_market(page)
    await taobao_shop_search(page, '森马')
    print('all is over')
    await page.close()
    time.sleep(1)
    await browser.close()
...
async def taobao_home_market(page):
    url = 'https://shopsearch.taobao.com/browse/shop_search.htm?'
    await page.goto(url)
    await page_evaluate(page)
    await taobao_x5_slide(page)
    await page.waitForSelector('#J_SearchForm', {'delay': input_time_random()})
    await page.waitFor(200)
    time.sleep(random.randint(1, 5))

async def taobao_shop_search(page, query_name):
    url = 'https://shopsearch.taobao.com/search?q={}&js=1&ie=utf8'.format(query_name)
    await page.goto(url)
    await page.reload()
    await taobao_x5_slide(page)
    await page.waitForSelector('#list-content', {'delay': input_time_random()})
    await page.waitFor(200)
...
async def special_page_wcloud(page, reportid=1278911):
    ...
    data_df.to_excel('d:\\test\\男装-词云-转化率.xlsx', index=False)
...
async def taobao_x5_slide(page):
