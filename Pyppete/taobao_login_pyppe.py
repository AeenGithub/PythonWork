...
async def taobao_login(username, password, url):
    ...
    browser = await launch({
        ...
        'args': ['--no-sandbox','--disable-infobars', '--start-maximized', '--window-size=1920, 1080', '--proxy-server=http://127.0.0.1:9000']
    })
    page = await browser.newPage()
    await page.setViewport({'width': 1920, 'height': 1080})
    await page.setUserAgent(
        'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3440.106 Safari/537.36')
    await page.goto(url)
    ...
    if slider:
        ...
        await page.goto('https://shopsearch.taobao.com/search?q=%E7%89%9B%E4%BB%94%E8%A3%A4+%E7%94%B7&js=1&initiative_id=staobaoz_20191014&ie=utf8&isb=0&shop_type=&ratesum=jin')
        try:
            global wrong_pwd_error  # 检测是否是账号密码错误
            print("error_1:", wrong_pwd_error)
            wrong_pwd_error = await page.Jeval('.error', 'node => node.textContent')
