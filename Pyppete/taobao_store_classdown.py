...
async def get_store_info(page):
    with open(r"E:\SPiderFile\male_cowl_pants.csv", 'r', encoding='utf-8') as f:
        ...
        for one_row in total_lst:
            ...
            try:
                    # 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.75 Mobile Safari/537.36')
                store_url = r'https://bigeve8.taobao.com/'
                await asyncio.wait([
                    page.goto(store_url),
                    page.waitForNavigation()
                ])
#                点击并等待页面加载，点击宝贝列表
                category_url = 'https://bigeve8.taobao.com/category.htm?search=y&scene=taobao_shop'
...
#                               options={'delay': input_time_random()}),
                ...
                total_page_info = await page.select('#J_ShopSearchResult > div > div.shop-hesper-bd.grid > div.shop-filter > div.pagination.pagination-mini > span.page-info')
                if total_page:
