...
class GetBrand(object):
    def __init__(self):
        ...
        self.month_lst = ['201710', '201711', '201712', '201801', '201802']
    ...
    def login_qbt(self):
        ...
        self.brandriver.find_element_by_id(
            'LoginForm_username').send_keys('森马上海服饰')
        self.brandriver.implicitly_wait(10)
        self.brandriver.find_element_by_id(
            'LoginForm_password').send_keys('a123')
    ...
    def get_lst_of_brand(self):
        ...
        #     time.sleep(10)
            ...
            print('1', e)
        all_href = self.brandriver.find_elements_by_css_selector(
            '#set1 > tbody > tr > td > a')
    ...
    def get_link_excel(self):
        ...
        brand_df.to_excel('./情报通/result/brand_link.xlsx')
        with open('brand', 'wb') as f:
            f.write(pickle.dumps(brand_df))
        return df_clothe_clss
    ...
    def rename_xls(self, brand, category, month):
        p = pathlib.Path('E:\\brand_down')
        try:
            down_file = [i for i in p.iterdir()][0]
            new_name = 'E:\\情报通shop\\{0}_{1}_{2}.xls'.format(
                brand, category.replace('/', '&'), month)
            down_file.rename(new_name)
        except:
            pass
        s = '     ' + brand + ' ' + category + month + ' 重命名成功\n'
        return s
    ...
    def got_file_from_web(self, brand_name, month, clothe_clss, midclss):
        ...
        # print(thead.split(' '))
        picture = self.brandriver.find_elements_by_css_selector('#content > div.charts > div > div.grid-view > table > tbody > tr > td > img')
        img_lst = []
        for one in picture:
        ...
        item_id = [[one.text, one.get_attribute('href')] for one in tbody_link if 'id' in one.get_attribute('href')]
        ...
        last_table.to_excel('E:\\情报通shop\\{0}_{1}_{2}_{3}.xls'.format(brand_name, midclss, clothe_clss.replace('/', '&'), month))
        print('    {0}_{1}_{2}_{3}保存成功'.format(brand_name, midclss, clothe_clss.replace('/', '&'), month))
        return last_table
    ...
    def main(self):
        ...
        for one_category in df_brand.values:
            ...
            for month in self.month_lst:
                ...
                try:
                    one_month_table = self.got_file_from_web(brand_name, month, clothe_clss, midclss)
                except:
                    one_month_table = None
                concat_df = concat_df.append(one_month_table)
                concat_df.to_csv('./情报通/result/汇总.csv', mode='a')
                one_row = '    '.join([brand_name, clothe_clss, clothe_url])
                with open("E:\\shop_url.txt", 'a', encoding='utf-8') as f:
                    f.write(one_row)
                    f.write('\n')
                continue
        print('所有的店铺均已完成')
