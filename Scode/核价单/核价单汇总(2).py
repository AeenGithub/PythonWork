...
def get_all():       #获取所有的核价单名单和地址
    '''这个函数是进行核价单目录的处理'''
    path = r'D:\文档\IE科'
    return [os.path.join(path, i) for i in os.listdir(path) if "~$" not in i ]

#def get_fraction():
#    '''这个函数是进行核价单目录的处理'''
#    path = r'D:\核价单汇总\已经核完价的核价单'
#    name_lst = [os.path.join(path, i) for i in os.listdir(path)]
#    xb_res_2 = app.books.open(r'D:\核价单汇总\模板\18Q4齐色核价汇总.xlsx')
...
def emb_flower():           #获取印绣花的价格
    '''获取印绣花的价格'''
    a = df[2].tolist()
    s = [str(i) for i in a if i != None][6:]
    t = 200
    for j in s:
        if '印' in j:
            t = a.index(j)
...
def drop_dup():     #对数据去重处理
    xb_res_2 = app.books.open(r"D:\文档\19Q1齐色核价汇总.xlsx")
    ...
    df_dup['款号'] = df_dup['款号'].apply(lambda x: x[:-2] if '.' in x else x)
#    df_dup.drop_duplicates(subset=['款号', '色号'], keep='last', inplace=True)
#    os.remove(r'D:\核价单汇总\模板\18Q4齐色核价汇总.xlsx')
    df_dup.to_excel(r"D:\文档\19Q1齐色核价汇总.xlsx", index=False)
    print('刷新原有数据成功')
    print('合计数据一共有', len(df_dup), '行')
...
xb_res = app.books.open(r"D:\文档\19Q1齐色核价汇总.xlsx")
...
for i, x in enumerate(get_all()):
    ...
    try:
        xb = app.books.open(x)
        print(xb.name, '正在写入……')
        need_sheet = [i for i in xb.sheets if i.api.visible==-1 and i.range('c7').value is not None]
        for i in need_sheet:
            df = fun_df(i)
            title = GetData(i)
...
app.screen_updating = True
try:
    xb_res.save(r"D:\文档\19Q1齐色核价汇总.xlsx")
#    time.sleep(5)
    xb_res.close()
    print('文件夹里共有', len(get_all()), '个文件')
    print('原有数据共有', all_location-2, '行')
    print(drop_dup())
    print('所有的数据均已写入，请查看')
