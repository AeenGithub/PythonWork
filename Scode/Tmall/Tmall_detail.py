...
class Producs_detail(object):
    ...
    def make_file(self, shop, item_id, title):
        """创建一个含有标题的表格"""
        if os.listdir('E:\\宝贝详情') is True:
            print('E:\\宝贝详情文件夹不为空')
            os.rmdir('E:\\宝贝详情')
            os.mkdir('E:\\宝贝详情')
        else:
            pass
        filename = 'E:\\宝贝详情\\{}_{}_{}.csv'.format(shop, item_id, self.datenw)
    ...
    def get_shop_lst(self):
        ...
        with open(self.pathfile, 'r', newline='', encoding='gbk') as f:
            for one in f.readlines():
                shop_set.add(one.split('\t')[0])
        return shop_set
                
    def set_resquests(self, url):
        thatid = url[-12:].strip(' ')
        datenum = int((datetime.now()-datetime.strptime('1970-01-01 00:00:00','%Y-%m-%d %H:%M:%S')).total_seconds()*1000)
        headers = {'user-agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWeb\
        Kit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1',
