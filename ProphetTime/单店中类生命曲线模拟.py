...
def one_senior_data_clean(main_store_num, one_senior):
    ...
    one_store = one_store[one_store['year'].isin(need_years)]
...
def main():
    ...
    for main_store_num in can_use_store:
        ...
        for one_senior in percent_up_95:
            ...
            one_store_senior = one_senior_data_clean(main_store_num, one_senior)
            try:
                bg_month_res, bg_day_res, ed_month_res, ed_day_res = get_bg_ed(one_store_senior)
    ...
    result_df.to_excel('d:\\test\\全国门店的各个中类的上下市时间.xlsx', index=False)
    result_df.to_pickle('configFile/store_market_time_begin_end_points_china.pickle')
